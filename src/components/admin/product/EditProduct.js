...
const EditProduct = (props) => {
    ...
    const handleCheckbox = (event) => {
        event.persist();
        setCheckboxs({...allcheckbox, [event.target.name] : event.target.checked});
    }

    useEffect(() => {
       axios.get(`/api/all_category`).then(response => {
       ...
       axios.get(`/api/edit_product/${product_id}`).then(response => {
           if(response.data.status === 200){
               setProduct(response.data.product);
               setCheckboxs(response.data.product);
           }
    ...
    const updateProduct = (event) => {
        ...
        axios.post(`/api/update_product/${product_id}`, formData).then(response => {
            if(response.data.status === 200){
    ...
    if(loading){
        <h4 className="text-center">Edit Product Data Loading...</h4>
    }
    ...
    <div className="container-fluid px-4">
        <div className="card mt-4">
            <div className="card-header">
               <h4>
               Edit Product
                   <Link to="/admin/view_product" className="btn btn-primary btn-sm float-end">View Product</Link>
               </h4>
            </div>
